...
public class Endecrypt {
	private static final Logger log = Logger.getLogger(Endecrypt.class);
	...
	private byte[] getEnKey(String strSrc) {
		byte[] desKey = null;
		try {
			MessageDigest md5 = MessageDigest.getInstance("MD5");
			byte[] desKeyTemp = md5.digest(strSrc.getBytes(encode));
			
			desKey = new byte[24];
			int i = 0;
			while (i < desKeyTemp.length && i < 24) {
				desKey[i] = desKeyTemp[i];
				i++;
			}
			if (i < 24) {
				desKey[i] = 0;
